defmodule Talar.PathsTest do
  ...
  describe "directories" do
    ...
    test "list_directories/1 should return [], if there is no parent direcrory" do
      assert Paths.list_directories("/drab") == []
    end

    test "list_directories/1 which belong to the given parent directory '/'" do
      directory = Talar.Repo.insert!(%Directory{path: "/a", directory_path: "/"})
      # IO.inspect(Paths.list_directories("/"))
      [dir] = Paths.list_directories("/")
      assert dir.path == directory.path
    end
